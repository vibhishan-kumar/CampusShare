import React, { useState, useEffect } from 'react';
import { useSearchParams, Link } from 'react-router-dom';
import { api } from '../api/client';
import StatusBadge from '../components/StatusBadge';
import PaymentModal from '../components/PaymentModal';
import FeedbackModal from '../components/FeedbackModal';
import { 
  Package, 
  ArrowDownLeft, 
  ArrowUpRight, 
  CheckCircle, 
  XCircle, 
  CreditCard, 
  RotateCcw, 
  Star, 
  MessageSquare, 
  ExternalLink, 
  Clock 
} from 'lucide-react';

export default function DashboardPage() {
  const [searchParams, setSearchParams] = useSearchParams();
  const initialTab = searchParams.get('tab') === 'lending' ? 'lending' : 'borrowing';
  const [activeTab, setActiveTab] = useState(initialTab);

  // Data states
  const [myListings, setMyListings] = useState([]);
  const [incomingRequests, setIncomingRequests] = useState([]);
  const [myBorrows, setMyBorrows] = useState([]);
  const [loading, setLoading] = useState(true);

  // Modals state
  const [paymentRequest, setPaymentRequest] = useState(null);
  const [feedbackRequest, setFeedbackRequest] = useState(null);
  const [actionRemarks, setActionRemarks] = useState({});

  const loadDashboardData = async () => {
    setLoading(true);
    try {
      const [listingsRes, incomingRes, borrowsRes] = await Promise.all([
        api.getMyListings(),
        api.getIncomingRequests(),
        api.getMyBorrows(),
      ]);

      if (listingsRes.success) setMyListings(listingsRes.items);
      if (incomingRes.success) setIncomingRequests(incomingRes.requests);
      if (borrowsRes.success) setMyBorrows(borrowsRes.requests);
    } catch (err) {
      console.error('Failed to load dashboard:', err);
    } finally {
      setLoading(false);
    }
  };

  useEffect(() => {
    loadDashboardData();
  }, []);

  const handleTabChange = (tab) => {
    setActiveTab(tab);
    setSearchParams({ tab });
  };

  // Owner actions: Accept or Reject
  const handleRequestAction = async (requestId, action) => {
    const remarks = actionRemarks[requestId] || '';
    try {
      const res = await api.updateBorrowStatus(requestId, { action, owner_remarks: remarks });
      if (res.success) {
        loadDashboardData();
      } else {
        alert(res.message || 'Action failed.');
      }
    } catch (err) {
      alert(err.message || 'Action failed.');
    }
  };

  // Owner action: Confirm Return Received
  const handleConfirmReturn = async (requestId) => {
    if (!window.confirm('Confirm that you have received the item back in good condition and returned the security deposit (if any)?')) {
      return;
    }
    try {
      const res = await api.confirmReturn({
        borrow_request_id: requestId,
        owner_note: 'Item received and verified on campus.',
        deposit_refunded: true,
      });
      if (res.success) {
        alert('Return confirmed! The item is now back to Available status.');
        loadDashboardData();
      }
    } catch (err) {
      alert(err.message || 'Failed to confirm return.');
    }
  };

  // Borrower action: Initiate Return
  const handleInitiateReturn = async (requestId) => {
    const note = prompt('Add an optional return note (e.g. Ready for pickup at LH-3 or CS Dept):', 'Ready to return');
    if (note === null) return;

    try {
      const res = await api.initiateReturn({
        borrow_request_id: requestId,
        borrower_note: note,
      });
      if (res.success) {
        alert('Return requested! Owner has been notified.');
        loadDashboardData();
      }
    } catch (err) {
      alert(err.message || 'Failed to initiate return.');
    }
  };

  return (
    <div>
      {/* Header */}
      <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '1.75rem', flexWrap: 'wrap', gap: '1rem' }}>
        <div>
          <h1 style={{ fontSize: '1.8rem', fontWeight: 800, color: 'var(--text-main)', marginBottom: '0.3rem' }}>
            Student Dashboard
          </h1>
          <p style={{ fontSize: '0.9rem', color: 'var(--text-muted)' }}>
            Track your campus item borrowings, approvals, payments, and listed items.
          </p>
        </div>
      </div>

      {/* Tabs */}
      <div style={{ display: 'flex', gap: '0.5rem', borderBottom: '1px solid var(--border-color)', marginBottom: '2rem' }}>
        <button
          type="button"
          onClick={() => handleTabChange('borrowing')}
          style={{
            padding: '0.75rem 1.25rem',
            fontWeight: 700,
            fontSize: '0.95rem',
            border: 'none',
            background: 'none',
            cursor: 'pointer',
            borderBottom: activeTab === 'borrowing' ? '3px solid var(--primary)' : '3px solid transparent',
            color: activeTab === 'borrowing' ? 'var(--primary)' : 'var(--text-muted)',
            display: 'flex',
            alignItems: 'center',
            gap: '0.5rem',
          }}
        >
          <ArrowDownLeft size={18} />
          Items I'm Borrowing
          <span style={{ background: '#f1f5f9', color: '#475569', fontSize: '0.75rem', padding: '0.1rem 0.45rem', borderRadius: '999px' }}>
            {myBorrows.length}
          </span>
        </button>

        <button
          type="button"
          onClick={() => handleTabChange('lending')}
          style={{
            padding: '0.75rem 1.25rem',
            fontWeight: 700,
            fontSize: '0.95rem',
            border: 'none',
            background: 'none',
            cursor: 'pointer',
            borderBottom: activeTab === 'lending' ? '3px solid var(--primary)' : '3px solid transparent',
            color: activeTab === 'lending' ? 'var(--primary)' : 'var(--text-muted)',
            display: 'flex',
            alignItems: 'center',
            gap: '0.5rem',
          }}
        >
          <ArrowUpRight size={18} />
          Items I'm Lending
          <span style={{ background: '#f1f5f9', color: '#475569', fontSize: '0.75rem', padding: '0.1rem 0.45rem', borderRadius: '999px' }}>
            {incomingRequests.length} requests
          </span>
        </button>
      </div>

      {loading ? (
        <div style={{ textAlign: 'center', padding: '3rem', color: 'var(--text-muted)' }}>Loading dashboard data...</div>
      ) : activeTab === 'borrowing' ? (
        /* ================= BORROWING TAB ================= */
        <div>
          {myBorrows.length === 0 ? (
            <div style={{ textAlign: 'center', padding: '3rem 1.5rem', background: '#ffffff', borderRadius: 'var(--radius-lg)', border: '1px solid var(--border-color)' }}>
              <Package size={40} color="#94a3b8" style={{ margin: '0 auto 1rem' }} />
              <h3 style={{ fontSize: '1.1rem', fontWeight: 700, marginBottom: '0.5rem' }}>No borrow requests sent yet</h3>
              <p style={{ fontSize: '0.85rem', color: 'var(--text-muted)', marginBottom: '1.25rem' }}>
                Need a calculator for exams, cycle for campus commute, or sports gear? Explore campus items.
              </p>
              <Link to="/browse" className="btn btn-primary">Browse Campus Items</Link>
            </div>
          ) : (
            <div style={{ display: 'flex', flexDirection: 'column', gap: '1.25rem' }}>
              {myBorrows.map((req) => (
                <div key={req.id} className="card" style={{ padding: '1.25rem' }}>
                  <div style={{ display: 'flex', flexWrap: 'wrap', justifyContent: 'space-between', alignItems: 'flex-start', gap: '1rem' }}>
                    {/* Item Info */}
                    <div style={{ display: 'flex', gap: '1rem', alignItems: 'center' }}>
                      <img
                        src={req.item_image || 'https://images.unsplash.com/photo-1584438784894-089d6a62b8fa?w=600'}
                        alt={req.item_name}
                        style={{ width: '65px', height: '65px', borderRadius: 'var(--radius-md)', objectFit: 'cover' }}
                      />
                      <div>
                        <h3 style={{ fontSize: '1.05rem', fontWeight: 700, marginBottom: '0.2rem' }}>
                          <Link to={`/items/${req.item_id}`}>{req.item_name}</Link>
                        </h3>
                        <div style={{ fontSize: '0.82rem', color: 'var(--text-muted)' }}>
                          Owner: <strong>{req.owner_name}</strong> ({req.owner_hostel || 'Hostel'})
                        </div>
                        <div style={{ fontSize: '0.8rem', color: '#475569', marginTop: '0.2rem' }}>
                          <Clock size={13} style={{ display: 'inline', marginRight: '3px' }} />
                          {req.start_date} to {req.end_date} • <strong>₹{req.total_price}</strong> rental
                          {req.item_deposit > 0 && ` + ₹${req.item_deposit} deposit`}
                        </div>
                      </div>
                    </div>

                    {/* Status & Actions */}
                    <div style={{ display: 'flex', flexDirection: 'column', alignItems: 'flex-end', gap: '0.6rem' }}>
                      <StatusBadge status={req.status} />

                      {/* State 1: ACCEPTED -> Pay Now */}
                      {req.status === 'ACCEPTED' && (
                        <button
                          type="button"
                          className="btn btn-success btn-sm"
                          onClick={() => setPaymentRequest(req)}
                        >
                          <CreditCard size={15} /> Proceed to Pay & Finalize
                        </button>
                      )}

                      {/* State 2: BORROWED -> Request Return */}
                      {req.status === 'BORROWED' && (
                        <button
                          type="button"
                          className="btn btn-primary btn-sm"
                          onClick={() => handleInitiateReturn(req.id)}
                        >
                          <RotateCcw size={15} /> Handover & Request Return
                        </button>
                      )}

                      {/* State 3: COMPLETED -> Leave Rating */}
                      {req.status === 'COMPLETED' && !req.my_feedback_id && (
                        <button
                          type="button"
                          className="btn btn-secondary btn-sm"
                          onClick={() => setFeedbackRequest(req)}
                        >
                          <Star size={15} color="#f59e0b" /> Rate Item & Owner
                        </button>
                      )}

                      {req.status === 'COMPLETED' && req.my_feedback_id && (
                        <span style={{ fontSize: '0.78rem', color: 'var(--success)', fontWeight: 700 }}>
                          ✓ Reviewed & Rated
                        </span>
                      )}
                    </div>
                  </div>

                  {req.owner_remarks && (
                    <div style={{ background: '#f8fafc', padding: '0.6rem 0.85rem', borderRadius: 'var(--radius-sm)', marginTop: '0.85rem', fontSize: '0.82rem', color: '#475569' }}>
                      <strong>Owner Note:</strong> {req.owner_remarks}
                    </div>
                  )}
                </div>
              ))}
            </div>
          )}
        </div>
      ) : (
        /* ================= LENDING TAB ================= */
        <div>
          {/* Sub-section: Incoming Requests */}
          <div style={{ marginBottom: '2.5rem' }}>
            <h2 style={{ fontSize: '1.25rem', fontWeight: 800, marginBottom: '1rem', color: 'var(--text-main)' }}>
              Incoming Borrow Requests from Students ({incomingRequests.length})
            </h2>

            {incomingRequests.length === 0 ? (
              <div style={{ padding: '1.5rem', background: '#ffffff', borderRadius: 'var(--radius-md)', border: '1px solid var(--border-color)', color: 'var(--text-muted)', fontSize: '0.88rem' }}>
                No active requests for your listed items right now.
              </div>
            ) : (
              <div style={{ display: 'flex', flexDirection: 'column', gap: '1rem' }}>
                {incomingRequests.map((req) => (
                  <div key={req.id} className="card" style={{ padding: '1.25rem' }}>
                    <div style={{ display: 'flex', flexWrap: 'wrap', justifyContent: 'space-between', alignItems: 'flex-start', gap: '1rem' }}>
                      <div>
                        <div style={{ fontSize: '0.75rem', fontWeight: 700, color: 'var(--primary)', textTransform: 'uppercase', marginBottom: '0.2rem' }}>
                          Request for: {req.item_name}
                        </div>
                        <div style={{ fontSize: '0.92rem', fontWeight: 700, color: 'var(--text-main)' }}>
                          Borrower: {req.borrower_name} ({req.borrower_department || 'Student'})
                        </div>
                        <div style={{ fontSize: '0.8rem', color: 'var(--text-muted)' }}>
                          Hostel: {req.borrower_hostel || 'Campus'} • Period: {req.start_date} to {req.end_date}
                        </div>
                        {req.request_note && (
                          <div style={{ fontSize: '0.82rem', color: '#475569', background: '#f8fafc', padding: '0.4rem 0.6rem', borderRadius: 'var(--radius-sm)', marginTop: '0.5rem' }}>
                            "{req.request_note}"
                          </div>
                        )}
                      </div>

                      <div style={{ display: 'flex', flexDirection: 'column', alignItems: 'flex-end', gap: '0.5rem' }}>
                        <StatusBadge status={req.status} />

                        {/* Owner Decision buttons for PENDING request */}
                        {req.status === 'PENDING' && (
                          <div style={{ display: 'flex', gap: '0.5rem' }}>
                            <button
                              type="button"
                              className="btn btn-success btn-sm"
                              onClick={() => handleRequestAction(req.id, 'ACCEPT')}
                            >
                              <CheckCircle size={15} /> Accept
                            </button>
                            <button
                              type="button"
                              className="btn btn-danger btn-sm"
                              onClick={() => handleRequestAction(req.id, 'REJECT')}
                            >
                              <XCircle size={15} /> Decline
                            </button>
                          </div>
                        )}

                        {/* Return requested by borrower -> Owner confirms */}
                        {(req.status === 'RETURN_REQUESTED' || req.status === 'BORROWED') && (
                          <button
                            type="button"
                            className="btn btn-primary btn-sm"
                            onClick={() => handleConfirmReturn(req.id)}
                          >
                            <CheckCircle size={15} /> Confirm Return Received
                          </button>
                        )}

                        {/* Review borrower when completed */}
                        {req.status === 'COMPLETED' && !req.my_feedback_id && (
                          <button
                            type="button"
                            className="btn btn-secondary btn-sm"
                            onClick={() => setFeedbackRequest(req)}
                          >
                            <Star size={15} color="#f59e0b" /> Rate Borrower
                          </button>
                        )}
                      </div>
                    </div>
                  </div>
                ))}
              </div>
            )}
          </div>

          {/* Sub-section: My Listed Items */}
          <div>
            <h2 style={{ fontSize: '1.25rem', fontWeight: 800, marginBottom: '1rem', color: 'var(--text-main)' }}>
              My Listed Items ({myListings.length})
            </h2>

            {myListings.length === 0 ? (
              <div style={{ padding: '2rem', background: '#ffffff', borderRadius: 'var(--radius-md)', border: '1px solid var(--border-color)', textAlign: 'center' }}>
                <p style={{ color: 'var(--text-muted)', marginBottom: '1rem' }}>You have not listed any items yet.</p>
                <Link to="/items/add" className="btn btn-primary btn-sm">List Your First Item</Link>
              </div>
            ) : (
              <div className="grid-cards">
                {myListings.map((item) => (
                  <div key={item.id} className="card" style={{ padding: '1rem' }}>
                    <img
                      src={item.image_url || 'https://images.unsplash.com/photo-1584438784894-089d6a62b8fa?w=600'}
                      alt={item.name}
                      style={{ width: '100%', height: '140px', objectFit: 'cover', borderRadius: 'var(--radius-md)', marginBottom: '0.75rem' }}
                    />
                    <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '0.4rem' }}>
                      <span style={{ fontSize: '0.75rem', fontWeight: 700, color: 'var(--primary)' }}>{item.category_name}</span>
                      <StatusBadge 
                        status={item.is_available ? 'AVAILABLE' : 'BORROWED'} 
                        availableAfter={item.available_after} 
                      />
                    </div>
                    <h3 style={{ fontSize: '0.95rem', fontWeight: 700, marginBottom: '0.4rem' }}>
                      <Link to={`/items/${item.id}`}>{item.name}</Link>
                    </h3>
                    <div style={{ fontSize: '0.85rem', fontWeight: 800, color: 'var(--text-main)', marginBottom: '0.75rem' }}>
                      ₹{item.price_per_day}/day {item.deposit > 0 && `(₹${item.deposit} deposit)`}
                    </div>
                    <div style={{ display: 'flex', gap: '0.4rem' }}>
                      <Link to={`/items/edit/${item.id}`} className="btn btn-secondary btn-sm" style={{ flex: 1 }}>
                        Edit
                      </Link>
                      <Link to={`/items/${item.id}`} className="btn btn-primary btn-sm" style={{ flex: 1 }}>
                        View
                      </Link>
                    </div>
                  </div>
                ))}
              </div>
            )}
          </div>
        </div>
      )}

      {/* Payment Checkout Modal */}
      {paymentRequest && (
        <PaymentModal
          borrowRequest={paymentRequest}
          onClose={() => setPaymentRequest(null)}
          onPaymentSuccess={() => {
            loadDashboardData();
          }}
        />
      )}

      {/* Rating / Review Modal */}
      {feedbackRequest && (
        <FeedbackModal
          borrowRequest={feedbackRequest}
          onClose={() => setFeedbackRequest(null)}
          onFeedbackSuccess={() => {
            alert('Review submitted! Thank you.');
            loadDashboardData();
          }}
        />
      )}
    </div>
  );
}
